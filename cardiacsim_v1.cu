...
__global__ void PDE(double *e, double *e_prev, int with, int n, const double alpha)
{

  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int column = blockIdx.x * blockDim.x + threadIdx.x;
  //int idx = row*with + column + n + 2*(row+1) + 1;
...
__global__ void ODE1(double *e, double *r, int with, int n,
...
__global__ void ODE2(double *e, double *r, int with, int n, const double dt,
                     const double epsilon, const double M1, const double M2,
                     const double kk, const double b)
{
...
__global__ void Mirroring(double *e_prev, int with, int n)
{
...
{ 
  ...
  Mirroring<<<DimGrid2, DimBlock2>>>(d_E_prev, m,n);

  PDE<<<DimGrid2, DimBlock2>>>(d_E, d_E_prev, m, n, alpha);
  
  ODE1<<<DimGrid2, DimBlock2>>>(d_E, d_R, m, n, dt, kk, a);

  ODE2<<<DimGrid2, DimBlock2>>>(d_E, d_R, m, n, dt, epsilon, M1, M2,kk,b);  
}
